...
bindings on **Windows x86_64**: `org.hdfgroup:hdf5-native:2.2.0` and
`org.hdfgroup:hdf5-java-ffm:2.2.0` with classifier **`windows-x86_64`**. The delivery wires these
into the **multi-module Maven build** (properties, `dependencyManagement`, and/or module
dependencies with **OS-scoped activation** so non-Windows builds are unchanged), updates
...
**`org.hdfgroup:hdf5-java-ffm:2.2.0:windows-x86_64`**  
...
Windows x86_64  
**Target Platform**: **Windows x86_64** for the new artifact line; Linux/macOS keep current
`build.properties` / `hdf5.lib.dir` behavior until follow-on  
...
**Gate conclusion**: Proceed. Re-check after design: confirm **Windows-only** activation does not
break Linux CI; confirm **FR-003** (no silent fallback) via pinned versions and optional Enforcer
`requireUpperBoundDeps` / banned duplicates analysis.

### Post-design re-check

- **OS gating**: Use Maven `activation` (`os.family` windows, `os.arch` amd64/x86_64 per Maven
  rules) or equivalent profile so Linux/macOS CI does not require unpublished Central artifacts.
...
**org.hdfgroup** artifacts enter the Maven graph on **Windows x86_64**, with documentation for
resolver visibility. Legacy **`jarhdf5`** at **2.0.0** may remain until a follow-on removes it;
